...
│                               Web Page (dApp)                              │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │  window.ethereum (EIP-1193 + DIDComm API)                            │  │
│  │  - eth_requestAccounts(), eth_accounts, eth_chainId                  │  │
│  │  - personal_sign()                                                   │  │
│  │  - sendDidCommMessage(), getDidCommMessages(), getSentDidCommMessages│  │
│  └───────────────────────────────────────────────────────────────────────┘  │
└───────────────────────┬─────────────────────────────────────────────────────┘
                        │ postMessage (plaintext requests)
                        ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                     Chrome Extension (Service Worker)                       │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ Background Script (background/background.js)                          │  │
│  │  - Wallet identity & private key (demo only)                          │  │
│  │  - Permission management (origin gating)                              │  │
│  │  - DIDComm message handling                                           │  │
│  │  - 🔐 Crypto engine (WebCrypto)                                       │  │
│  │      • ECDH (P‑256) key agreement                                     │  │
│  │      • AES‑256‑GCM encryption/decryption (E2EE)                       │  │
│  │      • ECDSA secp256k1 signing & verification                         │  │
│  │  - Message inbox/sent storage & unread badge                          │  │
│  │  - HTTP client → /api/SendDidCommMessage (send encrypted envelope)    │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ Offscreen Document (offscreen/signalr-client.js)                     │  │
│  │  - Persistent SignalR WebSocket connection                           │  │
│  │  - JoinDid(walletDid) group subscription                             │  │
│  │  - Receives encrypted DidCommMessageReceived events                  │  │
│  │  - Forwards encrypted envelopes to background script                 │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
└───────────────────────┬─────────────────────────────────────────────────────┘
                        │ SignalR WebSocket (encrypted envelopes only)
                        ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                     ASP.NET Core Backend (backend/)                         │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ SignalR Hub (Hubs/DidCommHub.cs, /didcommhub)                        │  │
│  │  - JoinDid(did) subscribe client to DID group                        │  │
│  │  - Broadcast encrypted DidCommMessageReceived to recipient’s group   │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ REST API (Controllers/DidCommController.cs, /api/SendDidCommMessage) │  │
│  │  - Accepts encrypted DidCommMessage envelopes (no plaintext body)    │  │
│  │  - Publishes to Kafka topic (Services/KafkaProducerService.cs)       │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ Kafka Consumer Service (Services/KafkaConsumerService.cs)            │  │
│  │  - Consumes from didcomm-messages topic                              │  │
│  │  - Forwards encrypted envelopes via SignalR to recipient DID group   │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
└───────────────────────┬─────────────────────────────────────────────────────┘
                        │
                        ▼
...
│  - Stores encrypted DIDComm envelopes (no decryption)                      │
│  - Durable, scalable routing between API and SignalR consumer              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Message & Encryption Flow (High Level)

1. dApp calls `window.ethereum.sendDidCommMessage(to, body)` with **plaintext body**.
2. Background script:
   - Resolves recipient DID to public key (demo registry).
   - 🔐 Encrypts `body` with ECDH (P‑256) + AES‑256‑GCM.
   - 🔐 Signs envelope with ECDSA secp256k1.
   - Sends encrypted `DidCommMessage` envelope to `/api/SendDidCommMessage`.
3. Backend:
   - Treats message as opaque JSON (no access to plaintext).
   - Publishes envelope to Kafka and/or broadcasts via SignalR to recipient DID group.
4. Recipient wallet:
   - Receives encrypted envelope via offscreen SignalR client.
   - 🔐 Verifies signature and decrypts body in `background.js`.
   - Stores plaintext in inbox and optionally sends encrypted ACK back to sender.
